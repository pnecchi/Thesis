...
value functions and are typically based on the Bellman equations, which are 
recursive relationships that these functions must satisfy.
...
\begin{theorem}[Optimal Policy]
	...
	\end{enumerate}
\end{theorem}
An optimal policy can be found by acting greedily with respect to $Q_*$, i.e.
\begin{equation}
	\pi_*(s,a) = \delta_{\argsup_a Q_*(s,a)}
\end{equation}
